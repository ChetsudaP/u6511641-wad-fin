...
export default function Home() {
  ...
  const createCustomerOrUpdate = async (data) => {
    ...
    try {
      ...
      // Immediately add the new customer to the list
      ...
      // Optionally refetch customers for consistency with the server
      fetchCustomers();
    } catch (error) {
          ...
          <ul className="list-disc ml-8">
            {customers.map((c) => (
              <li key={c._id}>
                ...
                {c.name} - {c.dob} - Member Number: {c.memberNumber} - Interests: {c.interests}
              </li>
            ))}
          </ul>
        </div>
